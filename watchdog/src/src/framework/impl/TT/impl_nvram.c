  ...
  if ((nvram_fd = open(PATH_DEV_NVRAM, O_RDWR)) >= 0)
    {
      ...
      if ((nvram_buf = mmap(NULL, NVRAM_SPACE, PROT_READ, MAP_SHARED, nvram_fd,
          0)) != MAP_FAILED)
        {
...
int
nvram_file2nvram ( const char *varname , const char *filename )
{
  FILE *fp;
  int c, count;
  int i = 0, j = 0;
  ...
  *((mode_t *) mem + 1) = MAGICNUM;
  ...
  count = fread(mem + 2 * sizeof(mode_t), 1, sizeof(mem) - 2 * sizeof(mode_t),
      fp);
  if (!feof(fp))
    {
      fclose(fp);
 ...
 */
int
nvram_nvram2file ( const char *varname , const char *filename )
{
  int fnum;
  int c, tmp;
  int i = 0, j = 0;
  ...
  if ((fnum = open(filename, O_WRONLY | O_CREAT | O_TRUNC, *((mode_t *) mem)))
      < 0)
    {
...
char *
_z_nvram_get ( const char *name )
{
  return nvram_get(name);
}
int
_z_nvram_set ( const char *name , const char *value )
{
  return nvram_set(name, value);
}
int
_z_nvram_unset ( const char *name )
{
  return nvram_unset(name);
}
int
_z_nvram_commit ( void )
{
  return nvram_commit();
}
